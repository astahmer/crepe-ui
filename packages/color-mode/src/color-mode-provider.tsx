...
const ColorModeContext = createContext<UseColorModeProps | undefined>(
  undefined
);
...
export const useColorMode = () =>
  useContext(ColorModeContext) ?? defaultContext;
...
const ColorMode: React.FC<ColorModeProviderProps> = ({
  ...
  const applyColorMode = useCallback((colorMode?: string) => {
    ...
    const d = shadowRoot
      ? shadowRoot.getElementById(rootId) ?? document.documentElement
    ...
    if (enableColorScheme) {
      const fallback = colorSchemes.includes(defaultColorMode)
        ? defaultColorMode
        : null;
  ...
  const providerValue = useMemo(
    () => ({
      ...
      systemColorMode: (enableSystem ? resolvedColorMode : undefined) as
        | "light"
        | "dark"
        | undefined,
    }),
    ...
    const scriptSrc = (() => {
      ...
      if (enableSystem) {
        return `!function(){try{${optimization}var e=localStorage.getItem('${storageKey}');if('system'===e||(!e&&${defaultSystem})){var t='${MEDIA}',m=window.matchMedia(t);if(m.media!==t||m.matches){${updateDOM(
          ...
          !defaultSystem
            ? `else{` + updateDOM(defaultColorMode, false, false) + "}"
            : ""
